...
pub fn delete_old<T>(path: T) -> Result<(), anyhow::Error>
    where T: AsRef<Path> + std::fmt::Debug
{
    ...
    Ok(if p.exists() {
        ...
        if p.metadata().context("Getting socket metadata")?.file_type().is_socket() {
            debug!("Deleting old socket file");
            std::fs::remove_file(p).context("Deleting socket file")?;
        } else {
            bail!("Socket path exists but it's not a Unix socket. Refusing to delete.");
        }
    })
}

pub async fn wait_for(path: &Path, max_secs: f32) -> anyhow::Result<()> {
