...
use crate::{
    api_server::server_state::ServerState,
    grpc::{caller::OrganizerCaller, grpc_client::OrganizerURI},
};
use anyhow::Context;
use database::{
    db_backup::{backup_sqlite_database, restore_sqlite_database},
    migration_solver::MigrationGraphModule,
    sqlite_foreign_key_check, DB,
};
use lib_clapshot_grpc::{
    proto::org::{self, Migration},
    GrpcBindAddr,
};
...
impl ClapshotInit {
    ...
    pub fn init_and_spawn_workers(
        ...
        use signal_hook::flag; // Work queue
        ...
        let api_thread = Some({
            ...
            let grpc_srv = if (&organizer_uri).is_some() {
                Some(grpc_server_bind.clone())
            } else {
                None
            };
        ...
        match &organizer_uri {
            Some(ouri) => {
                ...
                org.blocking_handshake_organizer(
                    &data_dir,
                    &url_base,
                    &db_file,
                    &grpc_server_bind,
    ...
    pub fn wait_for_termination(&mut self) -> anyhow::Result<()> {
        ...
        self.vpp_thread
            ...
            .expect("VPP thread failed");
        self.api_thread
            .take()
            .and_then(|t| t.join().ok())
            .expect("API thread failed");
        Ok(())
    }
}
...
fn migrate_db(db_file: &PathBuf, org_uri: &Option<OrganizerURI>) -> anyhow::Result<()> {
    ...
    let db: Arc<DB> =
        Arc::new(database::DB::open_db_file(&db_file).context("Error opening DB file")?);
    ...
    let server_module = {
        ...
        let server_migs = pending_server_migrations
            .iter()
            .map(|(m_name, m_version)| {
                let mig = Migration {
                    uuid: m_name.clone(),
        ...
        tracing::debug!(
            "Clapshot server has {} pending migrations.",
            server_migs.len()
        );
    ...
    let org_db_info = Some(org::Database {
        ...
        endpoint: db_file
            .canonicalize()?
            .to_str()
            .ok_or(anyhow::anyhow!("Sqlite path is not valid UTF-8"))?
            .into(),
    });
    ...
    if let Some(uri) = org_uri {
        ...
        let (rt, mut org_conn) = caller
            .tokio_connect()
            .context("Error connecting to Organizer")?;
        tracing::debug!("Calling check_migrations on Organizer.");

        match rt.block_on(org_conn.check_migrations(CheckMigrationsRequest {
            ...
            Err(e) => match e.code() {
                tonic::Code::NotFound => {
                    tracing::info!("No pending migrations from Organizer.");
                }
                tonic::Code::Unimplemented => {
                    tracing::info!("Organizer does not implement migrations. Ignoring.");
                }
                _ => {
                    anyhow::bail!("Error checking Organizer migrations: {:?}", e);
                }
            },
        }
    };

    match migration_modules
        .iter()
        ...
        .sum::<usize>()
    {
        ...
        n => {
            tracing::debug!("Total {} migrations to consider. Solving dependencies.", n);
        }
    }

    // Solve migration order
    let migration_order =
        database::migration_solver::solve_migration_graph(migration_modules.iter().collect())?;
    match migration_order {
        None => {
            tracing::error!(
                "Failed to solve migration dependencies. List of considered migrations:"
            );
        ...
        Some(order) => {
            ...
            .and_then(|_| {
                db.conn()
                    .context("Error opening DB connection after migrations")
            })
            .and_then(|mut conn| {
                sqlite_foreign_key_check(&mut conn, true)
                    .context("Foreign key checks failed after migrations")
            }) {
                Ok(_) => {
...
fn apply_migrations(
    ...
    let uuid_to_mod: HashMap<String, String> = migration_modules
        .iter()
        .flat_map(|m| {
            m.migrations
                .iter()
                .map(|mig| (mig.uuid.clone(), m.name.clone()))
        })
        .collect();
    ...
    for mig in plan {
        match uuid_to_mod.get(mig.uuid.as_str()) {
            ...
            Some(module_name) => {
                ...
                if let Some(uri) = org_uri {
                    let (rt, mut org_conn) = OrganizerCaller::new(uri)
                        .tokio_connect()
                    ...
                    .map_err(|e| {
                        anyhow::anyhow!(
                            "Error applying organizer migration '{}': {:?}",
                            &mig.uuid,
                            e
                        )
                    })?;
                    ...
                    bail!(
                        "Organizer migration '{}' found but no organizer URI to connect to.",
                        &mig.uuid
                    );
                }
            }
            None => {
                bail!(
                    "Migration '{}' not found in modules. This should not happen.",
                    mig.uuid
                );
            }
        }
