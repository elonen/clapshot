...
impl OrganizerCaller {
    ...
    pub fn blocking_handshake_organizer(
        ...
        server_url: &str,
        db_file: &Path,
        backchannel: &GrpcBindAddr,
    ) -> anyhow::Result<()> {
        async fn async_call_handshake(
            conn: &mut OrganizerConnection,
            backchannel: &GrpcBindAddr,
            data_dir: &Path,
            server_url: &str,
            db_file: &Path,
            ...
            let req = proto::org::ServerInfo {
                ...
                backchannel: Some(org::server_info::GrpcEndpoint {
                    endpoint: Some(match backchannel {
                        GrpcBindAddr::Tcp(addr) => org::server_info::grpc_endpoint::Endpoint::Tcp(
                ...
                version: Some(proto::org::SemanticVersionNumber {
                    major: v.major,
                    minor: v.minor,
                    patch: v.patch,
                }),
        ...
        for retry in 1..(MAX_TRIES + 1) {
            match self.tokio_connect() {
                Ok((rt, mut conn)) => {
                    ...
                    return rt.block_on(async_call_handshake(
                        &mut conn,
                        backchannel,
                        data_dir,
                        server_url,
                ...
                Err(e) => {
                    tracing::warn!(
                        "Connecting organizer failed (attempt {retry}/{MAX_TRIES}: {}",
                        e
                    );
