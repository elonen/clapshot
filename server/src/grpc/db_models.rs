...
use crate::database::{
    ...
    DBPaging, DbQueryByMediaFile, PooledConnection,
};
...
impl models::MediaFile {
    pub fn from_proto3(v: &proto::MediaFile) -> DBResult<Self> {
        Ok(Self {
            ...
            added_time: v
                .added_time
                .as_ref()
                .map(|t| proto3_to_datetime(t))
                .flatten()
                .ok_or(DBError::Other(anyhow::anyhow!("Bad added_time")))?,
            recompression_done: v
                .processing_metadata
                .as_ref()
                .map(|m| m.recompression_done.as_ref().map(|x| proto3_to_datetime(x)))
                .flatten()
                .flatten(),
            thumbs_done: v
                .processing_metadata
                .as_ref()
                .map(|m| m.thumbs_done.as_ref().map(|x| proto3_to_datetime(x)))
                .flatten()
                .flatten(),
            ...
            thumb_sheet_cols: v
                .preview_data
                .as_ref()
                .map(|d| d.thumb_sheet.as_ref().map(|x| x.cols as i32))
                .flatten(),
            thumb_sheet_rows: v
                .preview_data
                .as_ref()
                .map(|d| d.thumb_sheet.as_ref().map(|x| x.rows as i32))
                .flatten(),
            orig_filename: v
                .processing_metadata
                .as_ref()
                .map(|m| m.orig_filename.clone()),
            title: v.title.clone(),
            ...
            raw_metadata_all: v
                .processing_metadata
                .as_ref()
                .map(|m| m.ffprobe_metadata_all.clone())
                .flatten(),
            default_subtitle_id: v
                .default_subtitle_id
                .as_ref()
                .map(|id| {
                    id.parse()
                        .map_err(|_| DBError::Other(anyhow::anyhow!("Invalid default_subtitle_id")))
                })
                .transpose()?,
        })
    }
    ...
    pub fn to_proto3(
        ...
        let processing_metadata = match (
            &self.orig_filename,
            &self.recompression_done,
            &self.raw_metadata_all.clone(),
        ) {
            (Some(orig_filename), recompression_done, ffprobe_metadata_all) => {
                Some(proto::MediaFileProcessingMetadata {
        ...
        proto::MediaFile {
            ...
            subtitles: subtitles
                .into_iter()
                .map(|s| s.to_proto3(media_base_url))
                .collect(),
            default_subtitle_id: self.default_subtitle_id.map(|id| id.to_string()),
            playback_url: playback_uri
                .map(|uri| format!("{}/{}/{}", media_base_url, &self.id, uri)),
...
impl models::Subtitle {
    pub fn from_proto3(v: &proto::Subtitle) -> DBResult<Self> {
        let added_time = v
            .added_time
            .as_ref()
            .ok_or(anyhow::anyhow!("Missing added_time timestamp"))?;
        Ok(Self {
            id: v
                .id
                .parse()
                .map_err(|_| DBError::Other(anyhow::anyhow!("Invalid subtitle ID")))?,
            ...
            added_time: proto3_to_datetime(added_time)
                .ok_or(anyhow::anyhow!("Invalid 'added_time' timestamp"))?,
...
impl models::Comment {
    pub fn from_proto3(c: &proto::Comment) -> DBResult<Self> {
        ...
        let created = c
            .created
            .as_ref()
            .ok_or(anyhow::anyhow!("Missing created timestamp"))?;
        Ok(Self {
            id: c
                .id
                .parse()
                .map_err(|_| DBError::Other(anyhow::anyhow!("Invalid comment ID")))?,
            ...
            parent_id: c
                ...
                .map(|id| id.parse())
                .transpose()
                .map_err(|_| DBError::Other(anyhow::anyhow!("Invalid parent ID")))?,
            created: proto3_to_datetime(created)
                .ok_or(anyhow::anyhow!("Invalid 'created' timestamp"))?,
...
impl models::Message {
    pub fn from_proto3(v: &proto::UserMessage) -> DBResult<Self> {
        ...
        Ok(Self {
            id: id
                .parse()
                .map_err(|_| DBError::Other(anyhow::anyhow!("Invalid message ID")))?,
