...
use lib_clapshot_grpc::{subprocess::spawn_shell, subprocess::ProcHandle, unix_socket};
...
pub async fn connect(uri: OrganizerURI) -> anyhow::Result<OrganizerConnection> {
    let channel = match uri {
        ...
        OrganizerURI::Http(uri) => Channel::from_shared(uri.to_string())
            .context("Failed to parse organizer HTTP URI")?
            .connect_timeout(std::time::Duration::from_secs(8))
            .connect()
            .await
            .context("HTTP Channel::connect failed")?,
    };
...
pub fn prepare_organizer(
    ...
    let org_hdl = if let Some(cmd) = cmd {
        ...
        Some(spawn_organizer(
            &cmd.as_str(),
            org_uri.clone().unwrap(),
            debug,
            json,
        )?)
    } else {
