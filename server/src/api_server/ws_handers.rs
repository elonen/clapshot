...
use lib_clapshot_grpc::proto::client::client_to_server_cmd::{
    AddSubtitle, CollabReport, DelComment, DelMediaFile, DelSubtitle, EditComment,
    EditSubtitleInfo, JoinCollab, LeaveCollab, OpenMediaFile, OpenNavigationPage, RenameMediaFile,
    ReorderItems,
};
...
use super::user_session::{self, org_authz_with_default, AuthzTopic};
...
use crate::database::{
    models, DBPaging, DbBasicQuery, DbQueryByMediaFile, DbQueryByUser, DbUpdate, DB,
};
use crate::{
    client_cmd, optional_str_to_i32_or_tonic_error, send_user_error, send_user_ok,
    str_to_i32_or_tonic_error,
};
...
async fn get_media_file_or_send_error(
    media_file_id: Option<&str>,
    ses: &Option<&mut UserSession>,
    server: &ServerState,
) -> Res<Option<models::MediaFile>> {
...
pub async fn msg_open_navigation_page(
    data: &OpenNavigationPage,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    org_authz_with_default(
        &ses.org_session,
        "list media files",
        true,
        server,
        &ses.organizer,
        true,
        AuthzTopic::Other(None, authz_req::other_op::Op::ViewHome),
    )
    .await?;
        ...
        match org.lock().await.navigate_page(req).await {
            Err(e) => {
                if e.code() == tonic::Code::Unimplemented {
                    ...
                    tracing::debug!(
                        "Ignoring org.navigate_page() result because it GrpcStatus.ABORTED."
                    );
                    return Ok(());
                } else {
    ...
    for m in
        models::MediaFile::get_by_user(&mut server.db.conn()?, &ses.user_id, DBPaging::default())?
    {
        let subs = models::Subtitle::get_by_media_file(
            &mut server.db.conn()?,
            &m.id,
            DBPaging::default(),
        )?;
    ...
    let heading = proto::PageItem {
        item: Some(proto::page_item::Item::Html(h_txt.into())),
    };
...
pub async fn msg_open_media_file(
    data: &OpenMediaFile,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(v) =
        get_media_file_or_send_error(Some(&data.media_file_id), &Some(ses), server).await?
    {
        org_authz_with_default(
            &ses.org_session,
            "open media file",
            ...
            &ses.organizer,
            true,
            AuthzTopic::MediaFile(&v, authz_req::media_file_op::Op::View),
        )
        .await?;
...
pub async fn send_open_media_file_cmd(
    server: &ServerState,
    session_id: &str,
    media_file_id: &str,
) -> Res<()> {
...
pub async fn del_media_file_and_cleanup(
    media_file_id: &str,
    ses: Option<&mut UserSession>,
    server: &ServerState,
) -> Res<()> {
    tracing::info!(
        ...
        user_id = ses.as_ref().map(|u| u.user_id.clone()),
        "Trashing media file."
    );
    ...
    if let Some(v) = get_media_file_or_send_error(Some(media_file_id), &ses, server).await? {
        ...
        if let Some(ses) = &ses {
            ...
            org_authz_with_default(
                ...
                &ses.organizer,
                default_perm,
                AuthzTopic::MediaFile(&v, authz_req::media_file_op::Op::Delete),
            )
            .await?;
        }
        ...
        if let Some(ses) = ses {
            ...
            send_user_ok!(
                &ses.user_id,
                &server,
                Topic::MediaFile(&v.id),
                if !cleanup_errors {
                    format!("{} deleted.", media_type_str)
                } else {
                    format!("{} deleted, but cleanup had errors.", media_type_str)
                },
                details,
                true
            );
...
pub async fn msg_del_media_file(
    data: &DelMediaFile,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
...
pub async fn msg_rename_media_file(
    data: &RenameMediaFile,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(v) =
        get_media_file_or_send_error(Some(&data.media_file_id), &Some(ses), server).await?
    {
        ...
        if new_name.is_empty() || !new_name.chars().any(|c| c.is_alphanumeric()) {
            send_user_error!(
                ...
                server,
                Topic::MediaFile(&v.id),
                "Invalid file name (must have letters/numbers)"
            );
            return Ok(());
        }
        if new_name.len() > 160 {
...
pub async fn msg_add_comment(
    data: &proto::client::client_to_server_cmd::AddComment,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    let media_file_id =
        match get_media_file_or_send_error(Some(&data.media_file_id), &Some(ses), server).await? {
            Some(v) => {
                ...
                org_authz_with_default(
                    ...
                    &ses.organizer,
                    default_perm,
                    AuthzTopic::MediaFile(&v, authz_req::media_file_op::Op::Comment),
                )
                .await?;
                v.id
            }
    ...
    if let Some(d) = &drwn {
        if d.starts_with("data:") {
            ...
            let img_data = img_uri
                .decode_to_vec()
                .map_err(|e| anyhow!("Failed to decode drawing data URI: {:?}", e))?;
            ...
            async_std::fs::write(drawing_path, img_data.0)
                .await
                .map_err(|e| anyhow!("Failed to write drawing file: {:?}", e))?;
    ...
    ses.emit_new_comment(server, c, super::SendTo::MediaFileId(&media_file_id))
        .await?;
    Ok(())
}

pub async fn msg_edit_comment(
    data: &EditComment,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    ...
    match models::Comment::get(conn, &id) {
        Ok(old) => {
            ...
            org_authz_with_default(
                ...
                &ses.organizer,
                default_perm,
                AuthzTopic::Comment(&old, authz_req::comment_op::Op::Edit),
            )
            .await?;
            ...
            ses.emit_new_comment(server, c, super::SendTo::MediaFileId(&vid))
                .await?;
        }
        Err(DBError::NotFound()) => {
            send_user_error!(
    ...
    match models::Comment::get(conn, &id) {
        Ok(cmt) => {
            ...
            org_authz_with_default(
                ...
                &ses.organizer,
                default_perm,
                AuthzTopic::Comment(&cmt, authz_req::comment_op::Op::Delete),
            )
            .await?;
            ...
            if all_comm
                .iter()
                .any(|c| c.parent_id.map(|i| i.to_string()) == Some(id.to_string()))
            {
                send_user_error!(
                    &ses.user_id,
                    server,
                    Topic::MediaFile(&vid),
                    "Failed to delete comment.",
                    "Comment has replies. Cannot delete.",
                    true
                );
                return Ok(());
            }
...
pub async fn msg_add_subtitle(
        ...
        match get_media_file_or_send_error(Some(&data.media_file_id), &Some(ses), server).await? {
            Some(v) => {
                ...
                org_authz_with_default(
                    ...
                    &ses.organizer,
                    default_perm,
                    AuthzTopic::MediaFile(&v, authz_req::media_file_op::Op::Edit),
                )
                .await?;
                v
            }
    ...
    if !media_dir.exists() {
        bail!("Media file dir not found: {:?}", media_dir);
    }
    ...
    let orig_fn_clean: PathBuf = Path::new(&data.file_name)
        .file_name()
    ...
    if orig_sub_file.exists() {
        send_user_error!(
            &ses.user_id,
            server,
            Topic::MediaFile(&mf.id),
            "Failed to add subtitle.",
            format!("Subtitle file already exists: '{:?}'", &orig_fn_clean),
            true
        );
        return Ok(());
    }
    ...
    let file_contents = {
        ...
        STANDARD
            .decode(&data.contents_base64)
            .context("Failed to base64 decode subtitle file")?
    };

    tokio::fs::write(&orig_sub_file, file_contents)
        .await
        .context("Failed to write orig subtitle file")?;
    ...
    let playback_filename = {
        ...
        if vtt_path.exists() {
            send_user_error!(
                ...
                "Failed to add subtitle.",
                format!(
                    "WebVTT file already exists: '{:?}'",
                    &vtt_path.file_name().context("Bad filename")?
                ),
                true
            );
            return Ok(());
        ...
        match TimedSubtitleFile::new(&orig_sub_file) {
            Ok(TimedSubtitleFile::WebVtt(sub)) => {
                tracing::debug!(
                    "Subtitle file is already WebVTT, not converting: {:?}",
                    &orig_sub_file
                );
                None
            }
            Ok(sub) => {
                ...
                WebVttSubtitle::from(sub)
                    .export(&vtt_path)
                    .context("Failed to convert to WebVTT")?;
    ...
    let conn = &mut server.db.conn()?;
    let new_sub = models::Subtitle::insert(
        conn,
...
pub async fn msg_edit_subtitle_info(
    data: &EditSubtitleInfo,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    ...
    let mut sub =
        models::Subtitle::get(conn, &id).map_err(|e| anyhow!("Failed to get subtitle: {:?}", e))?;
    let mf = models::MediaFile::get(conn, &sub.media_file_id)
    ...
    org_authz_with_default(
        ...
        &ses.organizer,
        default_perm,
        AuthzTopic::MediaFile(&mf, authz_req::media_file_op::Op::Edit),
    )
    .await?;

    // Update subtitle in DB
    sub.title = data.title.clone().unwrap_or(sub.title.clone());
    sub.language_code = data
        .language_code
        .clone()
        .unwrap_or(sub.language_code.clone());
    sub.time_offset = data.time_offset.clone().unwrap_or(sub.time_offset);
    models::Subtitle::update_many(conn, &[sub])
        .map_err(|e| anyhow!("Failed to update subtitle: {:?}", e))?;
    ...
    if let Some(is_default) = data.is_default {
        ...
        if is_default || mf.default_subtitle_id == Some(id) {
            // only set null if this subtitle was previously the default
...
pub async fn msg_del_subtitle(
    ...
    if orig_path.exists() {
        std::fs::remove_file(&orig_path).context("Failed to delete orig subtitle file")?;
    }

    if let Some(vtt) = sub.filename {
        let vtt_path = subs_dir.join(&vtt);
        if vtt_path.exists() {
            std::fs::remove_file(&vtt_path).context("Failed to delete vtt subtitle file")?;
        }
    }

    models::Subtitle::delete(conn, &id)
        .map_err(|e| anyhow!("Failed to delete subtitle: {:?}", e))?;
    send_open_media_file_cmd(server, &ses.sid, &mf.id).await?;
    Ok(())
}

pub async fn msg_list_my_messages(
    data: &proto::client::client_to_server_cmd::ListMyMessages,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    let conn = &mut server.db.conn()?;
    ...
    for m in msgs {
        if !m.seen {
            models::Message::set_seen(conn, m.id, true)?;
        }
    }
    Ok(())
}

pub async fn msg_join_collab(
    data: &JoinCollab,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(collab_id) = ses.cur_collab_id.clone() {
        if server.sender_is_collab_participant(collab_id.as_str(), &ses.sender) {
            tracing::debug!(
                "{} is already in collab {}. Ignoring double join.",
                ses.user_name,
                collab_id
            );
            return Ok(());
        ...
        get_media_file_or_send_error(Some(&data.media_file_id), &Some(ses), server).await?
    {
        org_authz_with_default(
            ...
            AuthzTopic::Other(
                Some(&data.collab_id),
                authz_req::other_op::Op::JoinCollabSession,
            ),
        )
        .await?;
        ...
        match server.link_session_to_collab(&data.collab_id, &v.id, ses.sender.clone()) {
            ...
            Err(e) => {
                send_user_error!(
                    ...
                    server,
                    Topic::MediaFile(&v.id),
                    format!("Failed to join collab session: {}", e)
                );
            }
        }
    }
...
pub async fn msg_collab_report(
    data: &CollabReport,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(collab_id) = &ses.cur_collab_id {
        ...
        server
            .emit_cmd(ce, super::SendTo::Collab(collab_id))
            .map(|_| ())
    } else {
        send_user_error!(
            &ses.user_id,
            server,
            Topic::None,
            "Report rejected: no active collab session."
        );
        return Ok(());
    }
}

pub async fn msg_move_to_folder(
    data: &proto::client::client_to_server_cmd::MoveToFolder,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(org) = &ses.organizer {
        ...
        if let Err(e) = org.lock().await.move_to_folder(req).await {
            if e.code() == tonic::Code::Unimplemented {
                ...
                tracing::debug!(
                    "Ignoring org.move_to_folder() result because it GrpcStatus.ABORTED."
                );
            } else {
        ...
        send_user_error!(&ses.user_id, server, Topic::None, "No organizer session.");
    }
    Ok(())
}

pub async fn msg_reorder_items(
    data: &ReorderItems,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(org) = &ses.organizer {
        ...
        if let Err(e) = org.lock().await.reorder_items(req).await {
            if e.code() == tonic::Code::Unimplemented {
                ...
                tracing::debug!(
                    "Ignoring org.reorder_items() result because it GrpcStatus.ABORTED."
                );
            } else {
...
pub async fn msg_organizer_cmd(
    data: &proto::client::client_to_server_cmd::OrganizerCmd,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<()> {
    if let Some(org) = &ses.organizer {
        ...
        match org.lock().await.cmd_from_client(req).await {
            Err(e) => {
                if e.code() == tonic::Code::Aborted {
                    tracing::debug!(
                        "Ignoring org.cmd_from_client() result because it GrpcStatus.ABORTED."
                    );
                } else {
...
pub async fn msg_dispatch(
    req: &ClientToServerCmd,
    ses: &mut UserSession,
    server: &ServerState,
) -> Res<bool> {
    ...
    let res = match req.cmd.as_ref() {
        None => {
            send_user_error!(
                ...
                server,
                Topic::None,
                format!("Missing command from client: {:?}", req)
            );
            Ok(())
        }
        Some(cmd) => match cmd {
    ...
    if let Err(e) = res {
        ...
        if let None = e.downcast_ref::<user_session::AuthzError>() {
            ...
            let cmd_name = regex::Regex::new(r"^[a-zA-Z0-9_]+")
                .unwrap()
                .find(&cmd_str)
                .map(|m| m.as_str())
