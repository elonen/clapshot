    ...
    if let Some(parent_id) = c.parent_id {
        ...
        tokio::spawn(async move {
            let result: anyhow::Result<()> = async {
                ...
                // Lire la config SMTP depuis la DB (priorité) ou les env vars
                let smtp = match crate::email::SmtpConfig::from_db_or_env(&mut conn) {
                    Some(s) => s,
                    None => return Ok(()), // SMTP non configuré
                };

                let parent = models::Comment::get(&mut conn, &parent_id)?;
                let parent_user_id = match &parent.user_id {
                    Some(uid) => uid.clone(),
                    None => return Ok(()),   // utilisateur supprimé
                };
                ...
                let email_addr = match &parent_user.email {
                    ...
                    _ => return Ok(()),  // pas d'email configuré
                };
