...
use base64::{engine::general_purpose as Base64GP, Engine as _};

use super::user_session::OpaqueGuard;
use super::{
    Res, SendTo, SenderList, SenderListMap, SessionMap, StringToStringMap, UserSession, WsMsgSender,
};
...
impl ServerState {
    ...
    pub fn get_session_write<'a>(
        &'a self,
        sid: &str,
    ) -> Option<MappedRwLockWriteGuard<'a, UserSession>> {
    ...
    pub fn register_user_session(&self, sid: &str, user_id: &str, ses: UserSession) -> OpaqueGuard {
        let guard1 =
            self.add_sender_to_maplist(user_id, ses.sender.clone(), &self.user_id_to_senders);
    ...
    pub fn emit_cmd(
        &self,
        cmd: proto::client::server_to_client_cmd::Cmd,
        send_to: SendTo,
    ) -> Res<u32> {
        ...
        match send_to {
            ...
            SendTo::MediaFileId(media_file_id) => {
                self.send_to_all_media_file_sessions(media_file_id, &msg)
            }
            SendTo::MsgSender(sender) => {
                sender.send(msg)?;
                Ok(1u32)
            }
        }
    ...
    pub fn push_notify_message(
        &self,
        msg: &models::MessageInsert,
        send_to: SendTo,
        persist: bool,
        progress: Option<f32>,
    ) -> Res<()> {
    ...
    /// Bails out with error if any of the senders fail.
    /// Returns the number of messages sent.
    pub fn send_to_all_collab_users(
        &self,
    ...
    pub fn link_session_to_media_file(&self, session_id: &str, media_file_id: &str) -> Res<()> {
        ...
        let grd: OpaqueGuard = self.add_sender_to_maplist(
            media_file_id,
            ses.sender.clone(),
            &self.media_file_id_to_senders,
        );
    ...
    pub fn sender_is_collab_participant(&self, collab_id: &str, sender: &WsMsgSender) -> bool {
        ...
        senders
            ...
            .unwrap_or(&vec![])
            .iter()
            .any(|s| s.same_channel(sender))
    }

    pub fn link_session_to_collab(
        &self,
        collab_id: &str,
        media_file_id: &str,
        sender: WsMsgSender,
    ...
    /// Bails out with error if any of the senders fail.
    /// Returns the number of messages sent.
    pub fn send_to_all_media_file_sessions(
        &self,
        media_file_id: &str,
        msg: &super::Message,
    ) -> Res<u32> {
    ...
    fn add_sender_to_maplist(
        &self,
        key: &str,
        sender: WsMsgSender,
        maplist: &SenderListMap,
    ) -> OpaqueGuard {
                ...
                if !drawing.starts_with("data:") {
                    ...
                    if path.exists() {
                        ...
                        *drawing = format!(
                            ...
                            Base64GP::STANDARD_NO_PAD.encode(&data)
                        );
                    } else {
                    ...
                    tracing::warn!(
                        "Comment '{}' has data URI drawing stored in DB. Should be on disk.",
                        c.id
                    );
                }
            }
