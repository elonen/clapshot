...
mod integration_test {
    ...
    use crossbeam_channel::{select, unbounded, Receiver, RecvTimeoutError};
    ...
    use lib_clapshot_grpc::{proto, GrpcBindAddr};
    ...
    fn test_integ_metadata_reader_ok() -> anyhow::Result<()> {
        ...
        let args = IncomingFile {
            file_path: PathBuf::from_str(
                data_dir
                    .join("NASA_Red_Lettuce_excerpt.mov")
                    .to_str()
                    .unwrap(),
            )?,
        ...
        for i in 1..=MAX_RETRIES {
            ...
            if let Ok(resp) = resp_result {
                if resp.status() == 200 {
                    ...
                    tracing::debug!(
                        "wait_for_healthy got status {} from /api/health. Try {}/{}.",
                        resp.status(),
                        i,
                        MAX_RETRIES
                    )
    ...
    async fn wait_for_reports(
        ...
        check_file_outputs: Option<(PathBuf, String)>,
    ) -> WaitForReportResults {
        let mut res = WaitForReportResults {
            transcode_complete: false,
            thumbs_complete: false,
            got_progress_report: false,
            got_transcode_report: false,
            got_thumbnail_report: false,
            ts_cols: String::new(),
            ts_rows: String::new(),
        };
        ...
        for _ in 0..(60 * 2 * 10) {
            ...
            if still_waiting {
                match crate::api_server::test_utils::try_get_parsed::<ServerToClientCmd>(&mut ws)
                    .await
                    .map(|c| c.cmd)
                    .flatten()
                {
                    Some(s2c::Cmd::ShowMessages(m)) => {
                        // Got progress report?
                        res.got_progress_report |= m
                            .msgs
                            .iter()
                            .any(|msg| msg.r#type == proto::user_message::Type::Progress as i32);
                        ...
                        if m.msgs.iter().any(|msg| {
                            msg.r#type == proto::user_message::Type::MediaFileUpdated as i32
                        }) {
                            ...
                            else if m.msgs.iter().any(|msg| {
                                msg.clone().message.to_ascii_lowercase().contains("thumb")
                            }) {
                ...
                if still_waiting {
                    ...
                    println!(
                        "...waiting done, expected reports received. Doing OpenNavigationPage ..."
                    );
                    send_server_cmd!(
                        ws,
                        OpenNavigationPage,
        ...
        'waitloop: for _ in 0..80 {
            if reports_received_at.elapsed().as_millis()
                > (WAIT_AFTER_REPORTS_TIMEOUT_SECS * 1000).into()
            {
                panic!("Timeout checking API messages after transcode/thumbnail completion");
            }

            match crate::api_server::test_utils::expect_parsed::<ServerToClientCmd>(&mut ws)
                .await
                .cmd
            {
                Some(s2c::Cmd::ShowMessages(m)) => {
                    ...
                    assert!(
                        !m.msgs
                            .iter()
                            .any(|msg| msg.r#type == proto::user_message::Type::Error as i32),
                        "Got ERROR type message while waiting for ShowPage"
                    );
                }
                ...
                Some(s2c::Cmd::ShowPage(p)) => {
                    ...
                    if let Some(pm) = v.processing_metadata {
                        ...
                        if let Some(pd) = v.preview_data {
                            if let Some(thumb_url) = pd.thumb_url {
                                assert!(
                                    pm.thumbs_done.is_some(),
                                    "thumbs_done not set in processing metadata but got thumb_url"
                                );
                    ...
                    if (expect_thumbnail == res.thumbs_complete)
                        && (expect_transcode == res.transcode_complete)
                    {
                        break 'waitloop;
                    } else {
                ...
                something_else => {
                    tracing::info!(
                        "Got UNEXPECTED (not necessarily a bug) message: {:?}",
                        something_else
                    );
                }
            }
        ...
        tracing::info!(
            "Transcode complete: {} (expeted: {}), thumbs complete: {} (expected: {})",
            res.transcode_complete,
            expect_transcode,
            res.thumbs_complete,
            expect_thumbnail
        );
        ...
        if let Some((data_dir, input_filename)) = check_file_outputs {
            ...
            if expect_thumbsheet {
                ...
                assert!(thumb_dir
                    .join(format!("sheet-{}x{}.webp", res.ts_cols, res.ts_rows))
                    .is_file());
            }
            ...
            Ok(result) => {
                match result {
                    metadata_reader::MetadataResult::Ok(metadata) => {
                        ...
                        assert!(
                            metadata.duration > rust_decimal::Decimal::from(20),
                            "Duration should be > 20 seconds"
                        );
                        assert!(
                            metadata.duration < rust_decimal::Decimal::from(30),
    ...
    fn test_organizer_existing_v061_migrate() -> anyhow::Result<()> {
        match std::env::var("TEST_ORG_CMD").ok() {
            Some(org_cmd) => {
                ...
                let db_file = temp_dir.path().join("clapshot.sqlite");
                std::fs::copy(
                    "src/tests/assets/databases/clapshot-migration-test-2_v061.sqlite",
                    &db_file,
                )
                .expect("Failed to copy test DB for migration test");
                cs_main_test! {[ws, data_dir, incoming_dir, _org_conn, 500_000, Some(org_cmd), Some(temp_dir), IngestUsernameFrom::FileOwner]
    ...
    fn test_organizer_run_organizer_tests() -> anyhow::Result<()> {
        ...
        match std::env::var("TEST_ORG_CMD").ok() {
            Some(cmd) => {
                ...
                // listing fails.
                let log_path =
                    std::env::var("TEST_ORG_LOG").unwrap_or("organizer_tests.log".into());
                let log = Arc::new(Mutex::new(std::io::BufWriter::new(
                    std::fs::File::create(&log_path)
                        .expect(format!("Failed to create log file '{}'", &log_path).as_str()),
                )));
                ...
                let test_results: Arc<Mutex<Vec<(String, org::RunTestResponse)>>> =
                ...
                let mut test_names: Vec<String> = test_names
                    .lock()
                    .unwrap()
                    .iter()
                    .map(|s| s.clone())
                ...
                if let Some(filter) = std::env::var("TEST_ORG_FILTER")
                    .ok()
                    .filter(|s| !s.is_empty())
                {
                    write_log(
                        &log,
                        format!("    Filtering tests with pattern: '{}'", filter).as_str(),
                    );
                    test_names.retain(|name| name.contains(&filter));
                    if test_names.is_empty() {
                        write_log(
                            &log,
                            format!("    No tests match filter '{}'", filter).as_str(),
                        );
                ...
                write_log(
                    &log,
                    format!("    Running {} organizer tests", test_names.len()).as_str(),
                );

                for (i, test_name) in test_names.iter().enumerate() {
                    println!("\n\n\n------------ Running organizer test {}/{}: '{}'... ------------\n\n\n", i+1, test_names.len()+1, test_name);

                    let (_db, temp_dir, _videos, _comments) =
                        crate::database::tests::make_test_db();
                ...
                for (test_name, res) in test_results.iter() {
                    if let Some(err) = &res.error {
                        ...
                        write_log(
                            &log,
                            format!("\n---------------- RunTestResponse.output ----------------")
                                .as_str(),
                        );
                        write_log(&log, format!("{}", res.output).as_str());
                        write_log(
                            &log,
                            format!("\n---------------- RunTestResponse.error ----------------")
                                .as_str(),
                        );
                ...
                if test_results.iter().any(|(_, res)| res.error.is_some()) {
                    write_log(
                        &log,
                        format!("### Some organizer tests failed ###").as_str(),
                    );
                    panic!(
                        "Some organizer tests failed, output also logged into '{}'",
                        log_path
                    );
                }
            }
