...
pub fn solve_migration_graph(
    modules: Vec<&MigrationGraphModule>,
) -> anyhow::Result<Option<Vec<Migration>>> {
    ...
    for module in &modules {
        ...
        if let Some(max_version) = module
            .migrations
            .iter()
    ...
    let mut all_migrations: Vec<(&str, &Migration)> = modules
        .iter()
        .flat_map(|module| {
            module
                .migrations
                .iter()
                .filter(|mig| {
                    module.cur_version.is_none()
                        || mig.version.as_str() > module.cur_version.as_ref().unwrap().as_str()
                })
...
fn depth_first_search<'a>(
    ...
    if max_module_versions.iter().all(|(mod_name, max_ver)| {
        cur_module_versions
            .get(mod_name)
            .map_or(false, |cur_ver| cur_ver == max_ver)
    }) {
    ...
    macro_rules! migmod {
        ($name:expr, $cur_ver:expr, $migs:expr) => {
            MigrationGraphModule {
                name: $name.to_string(),
                cur_version: $cur_ver.map(|s: &str| s.to_string()),
                migrations: $migs,
            }
        };
    }
    macro_rules! mig {
        ($uuid:expr, $ver:expr, $deps:expr) => {
            Migration {
                ...
                dependencies: $deps,
                description: "dummy-desc".to_string(),
            }
        };
    }
    macro_rules! dep {
        ($name:expr, $min:expr, $max:expr) => {
            Dependency {
                name: $name.to_string(),
                min_ver: $min.map(|s: &str| s.to_string()),
                max_ver: $max.map(|s: &str| s.to_string()),
            }
        };
    }
    ...
    fn test_msolv_two_modules_nonsolvable() {
        ...
        let mod_org = migmod!(
            ...
            vec![
                ...
                mig!("G2", "2", vec![dep!("org", Some("2"), Some("2"))]),
                mig!(
                    "G3",
                    "3",
                    vec![
    ...
    fn test_actual_failing_case() {
        ...
        */
        let mod_server = migmod!(
            ...
            vec![
                mig!(
                    "2023-04-18-190209_change_video_primkey",
                    "2023-04-18-190209_change_video_primkey",
                    vec![dep!("clapshot.server", None, Some(""))]
                ),
                mig!(
                    "2023-04-18-190300_add_cascade_rules",
                    "2023-04-18-190300_add_cascade_rules",
                    vec![dep!(
                        "clapshot.server",
                        Some("2023-04-18-190209_change_video_primkey"),
                        Some("2023-04-18-190209_change_video_primkey")
                    )]
                ),
                mig!(
                    "2024-05-13-093800_add_users_table",
                    "2024-05-13-093800_add_users_table",
                    vec![dep!(
                        "clapshot.server",
                        Some("2023-04-18-190300_add_cascade_rules"),
                        Some("2023-04-18-190300_add_cascade_rules")
                    )]
                ),
                mig!(
                    "2024-05-22-163000_add_media_type",
                    "2024-05-22-163000_add_media_type",
                    vec![dep!(
                        "clapshot.server",
                        Some("2024-05-13-093800_add_users_table"),
                        Some("2024-05-13-093800_add_users_table")
                    )]
                ),
                mig!(
                    "2024-05-30-202000_add_missing_users",
                    "2024-05-30-202000_add_missing_users",
                    vec![dep!(
                        "clapshot.server",
                        Some("2024-05-22-163000_add_media_type"),
                        Some("2024-05-22-163000_add_media_type")
                    )]
                ),
            ]
