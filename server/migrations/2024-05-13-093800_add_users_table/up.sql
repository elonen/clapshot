...
-- !! Need to keep the old tables for now, so cascade rules don't delete anything before we've copied data from all the tables !!

-- Videos -- Remove the `user_name` column, and make `user_id` a foreign key column
...
-- Comments -- Rename user_name into username_ifnull, make current user_id a NULLABLE foreign key
...
-- Message -- Make user_id a foreign key
...
-- Step 6: Add indexes and triggers
