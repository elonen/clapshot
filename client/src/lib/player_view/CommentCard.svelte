...
function onClickDeleteComment() {
    var result = confirm($t("Delete comment?"));
...
function onClickShare() {
    ...
    try {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            ...
            }).catch(() => { alert($t("Copy link") + ": " + url); });
        } else { alert($t("Copy link") + ": " + url); }
    } catch (e) {
...
>
    ...
    <div class="p-2" lang="en">
        ...
        {:else}
            <p class="text-gray-300 text-base hyphenate">
                {comment.comment}
                {#if comment.edited}
                    <span class="text-xs italic text-gray-500"> {$t("(edited)")}</span>
                {/if}
            </p>
        {/if}
    </div>
    ...
    <div class="p-2 flex items-center justify-between" transition:slide="{{ duration: 200 }}">
        ...
        <div class="flex-1 flex justify-end">
            <button class="border rounded-lg px-1 placeholder: ml-2 text-sm border-cyan-500 text-cyan-500" onclick={()=>showReply=true}>{$t("Reply", { context: "comment", comment: "button to reply to a comment (verb, not the noun 'a reply')" })}</button>
            {#if comment.userId == $curUserId || $curUserIsAdmin}
                <button class="border rounded-lg px-1 ml-2 text-sm border-cyan-600 text-cyan-600" onclick={()=>{editing=true;}}>{$t("Edit", { context: "comment" })}</button>
                {#if !hasChildren()}
                <button class="border rounded-lg px-1 ml-2 text-sm border-red-300 text-red-300" onclick={onClickDeleteComment}>{$t("Del", { context: "comment", comment: "short label for the Delete button (abbreviation of 'Delete')" })}</button>
                {/if}
            {/if}
        </div>
    </div>
    ...
    <form class="p-2" onsubmit={(e) => {e.preventDefault(); onReplySubmit();}}>
            <input
                ...
                type="text" placeholder={$t("Your reply...")}
                use:callFocus
